...
namespace webtv_partition_editor
{
    public class ImDiskNamedPipeServer
    {
        ...
        public delegate void notify_unmount();
        ...
        public notify_unmount unmount_callback;
        private string mount_point;
        private string unmount_error_message;
        private string mount_error_message;
        private bool can_force_unmount = true;

        public ImDiskNamedPipeServer(WebTVPartition part, string mount_point = null, bool read_only = false, notify_unmount unmount_callback = null)
        {
        ...
        public void async_mount(string mount_point)
        {
            ...
            if (this.mount_point != null)
            {
                ...
                if (!mount_thread.Join(5000))
                {
                    this.internal_unmount();
                    this.stop_server();
                    throw new ApplicationException("Took too long to mount partition.");
                }
                else if (this.device_id == ImDiskNamedPipeServer.NO_DEVICE)
                {
                    this.stop_server();
                    throw new ApplicationException("Couldn't mount partition. " + this.mount_error_message);
                }
            }
        }
